...
const handler: ApiHandler = async (c) => {
	...
	const request = requestId ? await database.prepare(`SELECT id, issuer, state, nonce, code_verifier, return_path, expires_at FROM base_oidc_login_requests WHERE id = ?1`).bind(requestId).first<LoginRequest>() : undefined;
	...
	try {
		...
		let account = await database.prepare(`SELECT a.user_id, u.status FROM base_oidc_accounts a JOIN base_system_users u ON u.id = a.user_id WHERE a.issuer = ?1 AND a.subject = ?2`).bind(config.issuer, subject).first<{ user_id: number; status: string }>();
		if (!account) {
			...
			const user = await database.prepare('SELECT id, status FROM base_system_users WHERE username = ?1').bind(username).first<{ id: number; status: string }>();
		...
		const previousSession = await database.prepare(`SELECT session_id FROM base_oidc_sessions WHERE issuer = ?1 AND sid = ?2`).bind(config.issuer, oidcSessionId).first<{ session_id: string }>();
