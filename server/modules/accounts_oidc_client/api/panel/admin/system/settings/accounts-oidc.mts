...
const createFormPage = (issuerOptions: Array<{ value: string; text: string }>): FormPageConfig => ({
	...
	fields: [
		...
		{ name: 'issuer', label: 'Accounts Issuer', type: 'select', options: issuerOptions, placeholder: '选择 Passport 域名', rules: [{ required: true, message: '请选择 Accounts Issuer' }] },
...
const loadIssuerOptions = async (c: Parameters<ApiHandler>[0], currentIssuer: string) => {
	...
	const options = rows.filter((row) => !row.hostname.startsWith('*.')).map((row) => ({ value: `https://${row.hostname}`, text: `Passport (${row.hostname})` }));
	if (currentIssuer && !options.some((option) => option.value === currentIssuer)) options.unshift({ value: currentIssuer, text: `当前配置 (${currentIssuer})` });
	if (!options.length) options.push({ value: defaultIssuer, text: defaultIssuer });
	return options;
};
...
const handler: ApiHandler = async (c, next) => {
	...
	if (c.req.method === 'GET') {
		...
		return apiResponse(c, 200, { currentValues: { ...current, clientSecret: '' }, formPage: { ...createFormPage(issuerOptions), initialValues: { ...current, clientSecret: '' } } });
	}
	if (c.req.method === 'PUT') {
