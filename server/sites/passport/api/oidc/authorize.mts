...
const browserError = (c: Parameters<ApiHandler>[0], status: 400 | 404 | 503, message: string) => {
	...
	return c.html(renderAuthorizeError(message, `/accounts/sign${suffix}`), status);
};
...
const handler: ApiHandler = async (c) => {
	...
	if (requestId) {
		...
		if (!stored || stored.expires_at <= Date.now()) return browserError(c, 400, 'OIDC 授权请求不存在或已过期');
		values = stored;
	} else {
		if (c.req.query('response_type') !== 'code') return browserError(c, 400, '仅支持 response_type=code');
		values = {
	...
	if (!client || client.status !== 'enabled') return browserError(c, 400, 'OIDC 客户端不存在或已停用');
	...
	if (!registeredRedirectUris.includes(values.redirect_uri)) {
		...
		return browserError(c, 400, `redirect_uri 未注册：实际请求为 ${values.redirect_uri || '（空）'}；允许地址为 ${allowed}`);
	}
	const scopes = [...new Set(values.scope.split(/\s+/).filter(Boolean))];
	const allowed = new Set(client.allowed_scopes.split(/\s+/));
	if (!scopes.includes('openid') || scopes.some((scope) => !allowed.has(scope))) return browserError(c, 400, '请求的 scope 不被允许');
	...
	if (!current || pending) {
		...
		const signUrl = new URL(`/accounts/sign${c.get('techStackConfig').pageSuffix}`, c.req.url);
		if (c.req.query('popup') === '1') signUrl.searchParams.set('popup', '1');
