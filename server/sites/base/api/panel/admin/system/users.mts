...
const handler: ApiHandler = async (c, next, params) => {
	...
	if (c.req.method === 'GET' && !params.id) {
		...
		return apiResponse(c, 200, { table: { option: { rowKey: 'id', queryActions: [{ key: 'search', label: '搜索', action: 'search' }], toolbarActions: [{ key: 'create', label: '新增', action: 'create' }, { key: 'delete', label: '删除', action: 'delete' }], rowActions: [{ key: 'edit', label: '编辑', action: 'edit' }, { key: 'delete', label: '删除', action: 'delete' }] }, columns, dataSource: rows.results.map(publicUser), totalRecords: rows.results.length } });
	}
