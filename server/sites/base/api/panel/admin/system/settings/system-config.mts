...
const formPage = {
	...
	initialValues: { httpPort: '8088', domain: 'anan.cc', publicOrigin: '', trustedProxyIps: '', mapAllowedIps: '', logoutLocalEnabled: true, logoutPassportEnabled: true, logoutAllEnabled: true },
	fields: [
...
const handler: ApiHandler = async (c, next) => {
	...
	if (c.req.method === 'PUT') {
		...
		const next = mergeChangedFields(c.get('systemConfig'), body, ['httpPort', 'domain', 'publicOrigin', 'trustedProxyIps', 'mapAllowedIps', 'logoutLocalEnabled', 'logoutPassportEnabled', 'logoutAllEnabled']);
