...
const handler: ApiHandler = async (c, next) => {
	...
	if (c.req.method === 'GET') return c.json({ user: c.get('currentUser') ?? null, registrationAvailable: await registrationAvailable(database) });
	if (c.req.method === 'PUT') {
		if (!await registrationAvailable(database)) return c.json({ message: '初始管理员已经存在' }, 409);
		const credentials = await parseCredentials(c);
		if (!/^[a-zA-Z0-9_.-]{3,64}$/.test(credentials.username) || credentials.password.length < 8) {
			return c.json({ message: '用户名至少 3 个合法字符，密码至少 8 个字符' }, 400);
		}
		const now = Date.now();
		...
		if (Number(claimed.meta?.changes ?? 0) !== 1) return c.json({ message: '初始管理员已经存在' }, 409);
		try {
	...
	if (c.req.method === 'POST') {
		...
		if (!user || !await verifyPassword(credentials.password, user.password_hash)) return c.json({ message: '用户名或密码错误' }, 401);
