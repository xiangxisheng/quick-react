...
export default function WechatQrLogin({ commonApi }: { commonApi: CommonApi }) {
	...
	const load = () => {
		...
		commonApi.apiFetch('/api/accounts/external/wechat?format=json').then(async (response) => {
			...
			timerRef.current = window.setInterval(async () => { const result = await (await commonApi.apiFetch(data.pollUrl)).json(); if (result.status === 'authenticated') { stopPolling(); window.location.assign('/'); } else if (result.status === 'needs_email') { setBindUrl(result.bindUrl); setState((current) => current === 'code' ? 'code' : 'email'); setMessage('微信身份已确认，请验证邮箱'); } else if (result.status === 'expired') { stopPolling(); setState('expired'); setMessage('二维码已失效，请刷新后重新扫码'); } }, 2000);
