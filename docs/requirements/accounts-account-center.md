...
## 外部身份源合规要求（2026-08-27 补充）

Google 等身份源要求应用**向用户提供指向公共隐私权政策的链接**和**指向公共服务条款的链接**，OAuth 应用验证时也要在控制台填写同样的地址。

- 两个地址作为系统配置项维护（管理后台 → 系统管理 → 系统设置 → 系统配置），只接受 `http`/`https` 绝对地址，非法值按未配置处理。
- 配置后由后端随 `initialData.legalLinks` 下发，所有页面底部展示，登录页同样可见；未配置时不展示，也不下发字段。
- 覆盖测试：`npm run test:legal-links`。
