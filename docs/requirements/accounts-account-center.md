...
- 需要登录的地方（头部登录按钮、401 提示页）都在当前页面打开登录弹窗，不再把用户送去登录页。后端以 `action: 'accounts-login'` 表示 Accounts 弹窗，以 `action: 'local-login'` 表示本地账号密码弹窗。
- 公开 `/sign` 页面彻底取消，旧地址返回 404；`/api/sign` 仍作为本地登录弹窗的数据与提交接口。启用 Accounts 登录后本地注册入口一并隐藏（本地注册本来就会被拒绝）。
...
- 退出登录由当前站点后端撤销 Accounts 总会话并触发 OIDC Back-Channel Logout；成功响应通过通用 `next: { action: 'reload' }` 明确要求刷新当前页，前端只执行后端指令，不自行推断跳转目标或登录状态。
...
## 身份关联的判断准则（2026-08-28 确立）

以后遇到"这个身份要不要关联到已有账号、要不要先问用户"的问题，按下面五条判断，不需要再逐个讨论。

**一、外部身份已经关联过 `user_id` 时直接登录，这条优先于所有邮箱判断。** 对任何身份源都成立：微信、Telegram 这类不提供邮箱的身份源，只要该身份之前关联过账号，再次登录就直接建立会话，**不重新要求邮箱、也不发验证码**。邮箱只在"这个身份第一次出现、需要确定它属于谁"时才参与判断。

**二、判断依据是邮箱所有权是否已被证明，与身份源是谁无关。** 下面两种证明强度等价：

- 身份源自己给出已验证邮箱（Google 的 `email_verified`）；
- 用户手动填写邮箱并通过我方发出的验证码（微信、Telegram 等不提供邮箱的身份源走这条）。

**三、所有权已证明时，优先直接关联到已有账号并登录。** 不要求用户"先用原方式登录再绑定"：用户在那一刻没有别的选择，报错只是挫败。

**四、是否弹确认提示是体验选择，不是安全边界。** 静默关联和"该邮箱已是本站账号，继续将关联到该账号"的提示都可接受；不提示不构成安全问题。

**五、敢这么做的前提是结果可逆。** 被关联的身份都能在账户中心解绑，所以关联的代价是可逆的。设计新流程时沿用这条：**可逆的操作可以静默，不可逆的操作必须让用户确认。**

据此仍然不自动关联的情况，都是因为不可逆或前提不成立：

- 该外部身份已经绑定到另一个账号 —— 自动处理等于账号迁移，不可逆；
- 目标账号已停用 —— 前提不成立；
- 我方那条邮箱记录未验证（`verified = 0`）—— 所有权未被证明。

**两类身份源的区别只在"邮箱从哪来"**，不拆成两套流程：

| 类型 | 代表 | 邮箱来源 | 首次注册 | 再次登录 |
| --- | --- | --- | --- | --- |
| 邮箱型 | Google | 授权即得已验证邮箱 | 一步完成，不需要验证码，登录页标注推荐 | 直接登录 |
| 非邮箱型 | 微信、Telegram | 用户手动填写 + 我方验证码 | 需要先完成第三方认证，再验证邮箱 | 直接登录，不再要求邮箱和验证码 |
...
**决策**：当外部身份还没有绑定过，而身份源自己提供了**已验证邮箱**，且该邮箱已经是某个启用中账号的**已验证邮箱**时，直接把这个外部身份绑定到该账号并登录。

**理由**：身份源（Google）已经用自己的一整套验证流程证明了邮箱归属，我方这条邮箱记录也是发过验证码验证的，两边指向同一个邮箱所有者。此时再要求"先用原方式登录再绑定"，用户其实没有别的选择，只是多一次挫败——原来的做法是直接报错，用微信注册的老用户想改用 Google 登录时会撞墙。

**边界**（这些情况仍然不自动合并）：

- 身份源不提供邮箱（微信、Telegram 等，走我方验证码创建账号）时，邮箱已属于他人仍然拒绝，提示先登录原账户再绑定。
- 该外部身份已经绑定到另一个账号时拒绝，不做迁移。
- 目标账号已停用时拒绝。
