...
以后遇到"这个身份要不要关联到已有账号、要不要先问用户"的问题，按下面四条判断，不需要再逐个讨论。

**一、判断依据是邮箱所有权是否已被证明，与身份源是谁无关。** 下面两种证明强度等价：

- 身份源自己给出已验证邮箱（Google 的 `email_verified`）；
- 用户手动填写邮箱并通过我方发出的验证码（微信、Telegram 等不提供邮箱的身份源走这条）。

**二、所有权已证明时，优先直接关联到已有账号并登录。** 不要求用户"先用原方式登录再绑定"：用户在那一刻没有别的选择，报错只是挫败。

**三、是否弹确认提示是体验选择，不是安全边界。** 静默关联和"该邮箱已是本站账号，继续将关联到该账号"的提示都可接受；不提示不构成安全问题。

**四、敢这么做的前提是结果可逆。** 被关联的身份都能在账户中心解绑，所以关联的代价是可逆的。设计新流程时沿用这条：**可逆的操作可以静默，不可逆的操作必须让用户确认。**
...
| 类型 | 代表 | 邮箱来源 | 新用户注册 |
| --- | --- | --- | --- |
| 邮箱型 | Google | 授权即得已验证邮箱 | 一步完成，不需要验证码，登录页标注推荐 |
| 非邮箱型 | 微信、Telegram | 用户手动填写 + 我方验证码 | 需要先完成第三方认证，再验证邮箱 |
