...
业务站点跳到 Accounts 之后必须有回去的路：

- 业务站点个人中心的"前往账号中心"在**新标签页**打开，原页面不会离开。
- 登录页检测到待处理的 OIDC 授权请求时，说明"你正在为 <来源域名> 登录"，并提供"不登录了，返回 <来源域名>"的动作；返回地址取自数据库里已注册的 `redirect_uri` 的 origin，不接受外部传入，避免开放重定向。
- 业务站点的自动登录按后端配置的方式执行：`popup` 模式在弹窗里完成登录，本站页面保留；弹窗被拦截时展示提示并保留手动登录按钮，不再强制整页跳转。
