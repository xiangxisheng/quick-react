...
- **直接点第三方按钮**：
  - 微信 / Google → 跳转外部授权。**该外部身份已经绑定过 Accounts 用户的，直接建立会话登录，不再发送任何邮箱验证码**（即使身份源本身不提供邮箱，例如微信）；只有从未绑定过的新身份才需要邮箱验证码创建账号。
...
登录页、扫码页都可能开在业务站点的登录弹窗里，因此**任何登录成功的响应都要带上后续去向**（`redirectTo`），前端不得自行跳首页：

- 有待处理的 OIDC 授权请求 → 继续 `/api/oidc/authorize?request_id=...`，由授权端点判断用户名是否补全，最终回到业务站点并关闭弹窗。
- 没有待授权请求 → 用户名或密码还没补全就回登录页继续提示，都补全了才回首页。
...
- 页面：
  ...
  - **身份绑定**：列出已绑定的第三方账号和 Telegram 账号（来源、账号标识、绑定时间），支持解绑第三方账号。微信的 `subject` 是 `appid:openid`，列表只展示 openid。Telegram 账号提示到机器人里解绑。**解绑后必须还留有可用的登录方式**：只剩最后一个身份且没有设置密码时拒绝解绑。
  - **绑定身份**：独立页面，点击身份源按钮跳转授权，授权完成后回到身份绑定列表。返回页面通过短期 Cookie 记录，只接受账户中心内部路径，避免被引导到站外。
...
## 首页必须说明应用用途（2026-08-27 补充）

Google 应用验证要求首页公开说明应用的用途（[品牌验证要求](https://support.google.com/cloud/answer/13807376?hl=zh-Hans)）。原来的首页只渲染一句占位文本，无法通过验证。

- 首页内容由后端下发：`GET /api/home` 返回 `{ title, summary, sections, links }`，`base` 提供通用说明，`passport` 覆盖为 Accounts 账号中心的用途、登录方式、账号管理、统一登录范围和数据使用说明，并链接隐私权政策与服务条款。
- 站点首页导航节点的 `description` 同步改成用途说明，它同时用于 `<meta name="description">`。
- 文档 HTML 增加 `<noscript>` 回退，包含标题、用途说明和隐私权政策链接，保证不执行脚本的检查方也能读到。
- 首页不需要登录即可访问。
- 覆盖测试：`npm run test:home-page`。
