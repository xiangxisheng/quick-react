...
- 业务站点登录页展示说明文案（"点击下面的按钮会打开 Accounts 登录窗口，完成后自动回到本站；本页不会自动跳转"）和一个按钮，按钮文案由后端下发；页面加载时不发起任何登录请求，`passportLogin` 协议里不再有 `autoStart`。
- 点击后按后端配置的方式登录，默认 `popup`：弹窗内完成登录，业务页面始终保留；弹窗被拦截时显示提示，用户可以再点一次。
- 业务站点个人中心的"前往账号中心"在**新标签页**打开，原页面不会离开。
- 万一用户已经到了 Accounts 登录页（例如直接访问受保护页面被挡回来），登录页会说明"你正在为 <来源域名> 登录"，并提供"不登录了，返回 <来源域名>"的动作；返回地址取自数据库里已注册的 `redirect_uri` 的 origin，不接受外部传入，避免开放重定向。
