...
1. **第一步输入邮箱**，后端判断该邮箱是否已注册（已验证邮箱且用户启用）。
2. **已注册**：返回该邮箱真实可用的登录方式，用户选择其一：
   - 密码登录（该用户已设置密码时才出现）；
   - Telegram 消息批准（该用户有可用 Telegram 绑定时才出现）；
   - 已绑定的第三方身份（Google / 微信，按已启用的身份源）。
3. **未注册**：进入确认步骤，文案为"{email} 还没有注册，请确认邮箱地址是否正确"，提供"重新输入"和"确认无误，继续注册"。
   - 确认后把邮箱写入短期 HttpOnly cookie（30 分钟），进入第三方认证选择。
   - 第三方返回**已验证邮箱** → 直接创建 Accounts 用户并登录，不再要求绑定邮箱。
   - 第三方没有邮箱（微信）→ 用暂存邮箱预填验证码流程，验证通过后创建用户。
4. 登录成功后统一进入第二、三节的补全流程，再跳回原目标。

### OIDC 回跳约束

补全步骤会延长登录耗时，而 `passport_oidc_authorization_requests` 与 `accounts_oidc_request` cookie 的有效期都是 10 分钟。进入补全步骤时必须同时续期数据库行和 cookie；最终跳向 `/api/oidc/authorize` 的那一步仍然要清除 cookie，放弃补全直接访问首页的路径也要清除。

### 验收标准

- 未注册邮箱不再返回 404，而是进入确认步骤。
- 已注册邮箱只列出真实可用的登录方式。
- 微信（无邮箱）注册路径里验证码步骤的邮箱默认就是第一步输入的邮箱。
- Google（已验证邮箱）注册路径不再要求二次验证邮箱。
...
- 页面：
  ...
  - **邮箱管理**：邮箱列表（主邮箱、验证状态、绑定时间）；添加邮箱（发送验证码 → 输入验证码完成绑定）；设为主邮箱；解绑。主邮箱和最后一个邮箱不允许解绑。
