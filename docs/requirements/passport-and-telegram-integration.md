...
默认允许一个用户绑定多个邮箱和多个外部身份，但同一个已验证邮箱、同一机器人下的同一个 Telegram 用户、同一 Provider 下的同一个外部用户不能据此创建第二个 `user_id`。如果邮箱和外部身份分别命中不同的现有用户，必须让用户明确选择进入哪个账号或取消操作，不得自动合并。

> 2026-08-28 修订：身份源**自己提供已验证邮箱**（目前只有 Google 的 `email_verified`）且该邮箱已经是某个启用中账号的已验证邮箱时，改为直接把该外部身份绑定到那个账号并登录，不再要求用户先用原方式登录。理由见 [accounts-account-center](accounts-account-center.md#按已验证邮箱自动关联身份2026-08-28-决策)。其它情况（微信等不提供邮箱的身份源走我方验证码、外部身份已绑定到另一个账号）仍然保持不自动合并。邮箱、Telegram 和其他 Provider 的数量上限通过 Passport 设置项控制，默认不限制。
